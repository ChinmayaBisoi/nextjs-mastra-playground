 ...
 */
export function extractLayout(slideData: any, relsMap: RelationshipMap): SlideLayout {
  // Try to find layout reference in relationships
  const layoutRel = Array.from(relsMap.values()).find(
    (rel) => rel.type.includes("slideLayout")
  );
 ...
 */
export function extractPositionAndSize(spPr: any): { position: Position; size: Size } {
  const xfrm = spPr?.xfrm || {};
  const off = xfrm.off || {};
  const ext = xfrm.ext || {};
 ...
 */
export function extractTextFormatting(
  rPr: any,
  pPr: any
): TextFormatting {
  ...
  const alignment = pPr?.["@_algn"]
    ? getTextAlignment(pPr["@_algn"])
    : "left";
